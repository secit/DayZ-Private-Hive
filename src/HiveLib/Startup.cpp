...
namespace
{
	...
	vector<string> GetCmdLineParams(LPTSTR** argv=NULL, int* argc=NULL)
	{
		...
		if (argv) { *argv = cmdLineArgs; }
		if (argc) { *argc = numCmdLineArgs; }
...
namespace
{
	...
	unique_ptr<HiveExtApp> CreateApp()
	{
		...
		string serverFolder = "@dayzcc";

		for (auto it = cmdLine.begin(); it != cmdLine.end(); ++it)
		{
			// Get Profile folder where HiveExt.ini is located
			string starter = "-profiles=";
			if (it->length() < starter.length()) { continue; }
			string compareMe = it->substr(0, starter.length());
			if (!boost::iequals(compareMe, starter)) { continue; }
